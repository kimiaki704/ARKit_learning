//
//  ObjectDistance3ViewController.swift
//  ARKit_learning
//
//  Created by 鈴木公章 on 2019/09/06.
...
final class ObjectDistance3ViewController: UIViewController {
...
extension ObjectDistance3ViewController: ARSCNViewDelegate {
        ...
        if let imageAnchor = anchor as? ARImageAnchor {
            ...
            if (imageAnchor.referenceImage.name?.contains("car"))! && nodesArray.count >= 1 {
                if referenceImageName != imageAnchor.referenceImage.name {
                    ...
                    node.name = imageAnchor.referenceImage.name!
                    nodesArray[0].enumerateChildNodes { (child, _) in
    ...
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        if nodesArray.count == 2 {
            ...
            var positionTwo = SCNVector3ToGLKVector3(nodesArray[1].position)
            
            if nodesArray[1].name == "car_21" {
                positionTwo.z = 0
            }
